...
import ShippingRepository 
import Hateoas 

bp = Blueprint('shipping', __name__, url_prefix='/shippings')

@bp.route("", methods=["POST"])
def Post():
    response = ShippingRepository.Create(request)
...
def Get():
    response = ShippingRepository.Read(request)
...
def Get_By_Id(id: int):
    response = ShippingRepository.Read_by_id(id)
...
def Update(id: int):
    ...
    response = ShippingRepository.Update(id, request)
...
def Delete(id: int):
    response = ShippingRepository.Delete(id)
    if response == None:
